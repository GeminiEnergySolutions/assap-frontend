...
export class PhotoService {
  ...
  getPhotos(query: PhotoQuery): Observable<Response<{
    ...
    return this.http.get<Response<{
      photos: Photo[];
      count_total_photos: number;
    }>>(`/api/audit/${query.auditId}/photos`, {params});
  }

  deletePhoto(auditId: number, id: number): Observable<Response> {
    return this.http.delete<Response>(`/api/audit/${auditId}/photos/${id}`);
  }
  ...
  uploadPhoto(info: PhotoInfo, file: File): Observable<object> {
    ...
    return this.http.post<Response<{upload_url: string;}>>(`/api/audit/${info.auditId}/photos`, formData).pipe(
